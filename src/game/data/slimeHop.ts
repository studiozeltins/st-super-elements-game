// VERBATIM client mirror of spacetimedb/src/slimeHop.ts (the server is the
// source of truth — edit both together). The client animates the slime bounce
// from the enemy row's hop columns using these constants/helpers; a runtime
// import of the server module into the client bundle is rejected by project
// convention (D5-07), so slimeHop.test.ts parity-locks the two files instead.
