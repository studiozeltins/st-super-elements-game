...
- [x] 03-02-PLAN.md — deploy capstone: additive local publish (migrate-acceptance test for the event-table columns) → generate → build → two-client migrated-DB playtest (SC2/SC3, human checkpoint) (wave 2)
...
- [x] 04-01-PLAN.md — test-first pure helpers: `attacks.ts` (ATTACKS/UNIT_ATTACKS/selectAttack), `unitAttackFsm.ts` (deadline math, late-tick resolve), `attackHitbox.ts` (circle + knockback) + serverSync INV-5 extension

**Wave 2** *(blocked on Wave 1)*

- [x] 04-02-PLAN.md — additive schema (`unit_attack`, `attack_strike`, `player.stunnedUntilMicros` + stun guard) + `unitAttacks.ts` runUnitAttacks glue (lazy upsert, landing lock, grace resolution, knockback/stun)
...
- [x] 04-04-PLAN.md — createTelegraphSystem (outline + outward fill + rim flash, #86e2ff) + App.tsx/createGame unit_attack row plumbing

**Wave 5** *(blocked on Wave 4)*

- [x] 04-05-PLAN.md — optional `animateAttack` hook on EntityAnimation + goliath crouch/leap/slam procedural animation + attack-view plumbing

**Wave 6** *(blocked on Wave 5)*

- [x] 04-06-PLAN.md — strike juice (burst + camera shake + WebAudio slam SFX) on attack_strike onInsert + client stun/knockback reconcile
...
- [x] 05-02-PLAN.md — client clips (`animateSwing`/`animateSwirl`) + per-attack strike juice + `playSwing`/`playSwirl` SFX (wave 1)

**Wave 2**

- [x] 05-03-PLAN.md — glue wiring (chain swap+break, leap-only teleport gate, shape branch, per-role cooldown) + additive `basicCooldownUntilMicros` column + local migrate-publish + bindings (wave 2)
- [x] 05-04-PLAN.md — client mirror `src/game/data/attacks.ts` + serverSync parity + cone SECTOR telegraph + rebuild-on-attackId-change (wave 2)
...
- [x] 06-02-PLAN.md — client feel: `animateDash` grounded-lunge clip + `playDash` clang SFX + shieldDash strike-juice tier (burst + lane dust wake, no shockwave) (wave 1)
...
- [x] 06-04-PLAN.md — lane rectangle telegraph branch (cast-anchor + yaw, axis-only fill sweep via `fillAxis`, re-cast rebuild rule) (wave 2)
...
| 6. shieldDash lane | 4/5 | In Progress|  |
